...
// new templates:
// postbit_threaded_author_user, postbit_threaded_posturl, postbit_threaded_iplogged_hiden, postbit_threaded_posturl

/**
 * Build a post bit
 *
 * @param array $post The post data
 * @param int $post_type The type of post bit we're building (1 = preview, 2 = pm, 3 = announcement, 4 = reddit thread type post. else = linear post)
 * @return string The built post bit
 */
function build_postbit($post, $post_type=0, $indentsize = 0, $indentsize2 = 0, $indentsize3 = 0, $pnumber = 0, $post_bkg = -1, $post_bordercolor = 0, $lastreadtime = 0)
{
...
//	global $plugins, $parser, $cache, $ignored_users, $hascustomtitle,	$indentsize;

	$hascustomtitle = 0;
	
	$FLATUSERTITLE = ($post_type == 4) ? 1 : 0;
	...
	$unapproved_shade = '';
	if(isset($post['visible']) && $post['visible'] == 0 && ($post_type == 0 || $post_type == 4))
	{
		$altbg = $unapproved_shade = 'unapproved_post';
	}
	elseif(isset($post['visible']) && $post['visible'] == -1 && ($post_type == 0 || $post_type == 4))
	{
	...
	elseif($mybb->input['mode'] == "threaded")    // XXX
	{
		$post_extra_style = "border-top-width: 0;";
	}
	else
	...
	// Dont want any little 'nasties' in the subject   // XXX shouldn't there be a check?
	...
	$post['groupimage'] = '';
	if(!empty($usergroup['image']) && !$FLATUSERTITLE )
	{
	...
	if($post['userusername'])
	{
		...
		if($post['starimage'] && $post['stars'] && !$FLATUSERTITLE)
		{
	...
	if (!$FLATUSERTITLE) {
		...
		if($post_type != 3 && $mybb->settings['enablewarningsystem'] != 0 && $usergroup['canreceivewarnings'] != 0 && ($mybb->usergroup['canwarnusers'] != 0 || ($mybb->user['uid'] == $post['uid'] && $mybb->settings['canviewownwarning'] != 0)))
		{
			...
			if($mybb->usergroup['canwarnusers'] != 0 && $post['uid'] != $mybb->user['uid'] && ($post_type == 0 || $post_type == 2 || $post_type == 4))
			{
	...
	if (!$FLATUSERTITLE) {
		eval("\$post['user_details'] = \"".$templates->get("postbit_threaded_author_user")."\";");
		} else {
	...
	if ($FLATUSERTITLE) {
                $post_number = my_number_format($pnumber);
		eval("\$post['posturl'] = \"".$templates->get("postbit_threaded_posturl")."\";");
	} else {
	...
	if($post_type == 0 || $post_type == 2 || $post_type == 4)
	{
	...
	if ($FLATUSERTITLE) {
				...
				eval("\$post['iplogged'] = \"".$templates->get("postbit_threaded_iplogged_hiden")."\";");
	} else {
	...
	switch($post_type)
	{
		...
		case 3: // Announcement
			...
			break;
		default: // Regular post
			if ($post_type == 4) {	// reddit threaded style
			  $post = $plugins->run_hooks("postbit_threaded", $post);
			} else {
	...
	else if ($post_type == 4) 	// reddit threaded style
	{
		eval("\$postbit = \"".$templates->get("postbit_threaded")."\";");
	}
	else 
	{
