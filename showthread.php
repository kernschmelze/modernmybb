...
$lastreadtime = 0;  // XXX does this need to be global?
...
// XXX do this AFTER displaying, else no highlighting unread possible
// line commented and moved after posts processing
...
// get time thread was last read by user

        if ($mybb->user['uid']) {
  //      if (0) {

	  $tmpuuid = $mybb->user['uid'];
          $LASTREADTIMEquery = $db->query("
                SELECT dateline 
            FROM ".TABLE_PREFIX."threadsread
            WHERE tid='$tid'  AND uid='$tmpuuid'
            ORDER BY dateline
                ");
//          $lastreadtime = $db->fetch_array($LASTREADTIMEquery);
          $lastreadtime = $db->fetch_field($LASTREADTIMEquery, "dateline");
        }
    //      $lastreadtime = 9999999999;
...
// 		$upper = $start+$perpage;
        ...
        else
        {
            ...
            if($defaultmode != "threaded")
            {
                $threadmode = "&amp;mode=threaded";
            }
        }

        // TODO: Do the attachcache stuff here
        // left out as I don't allow attachments and thus cannot test them
...
//                 }
                ...
                $treepnumb[$post['replyto']][$post['pid']] = $pnumb++;
...
//         }
        ...
        // now build threaded post output from the tree
        
		$posts = buildtree($lastreadtime);
...
// XXX linear end 
	...
	if($mybb->get_input('mode') == 'threaded') {
	  eval("\$showthread = \"".$templates->get("showthread_threaded")."\";");
	} else {
 ...
 * @return string
 */
function buildtree($lastreadtime = 0, $replyto=0, $indent=0, $indentcolor = 0xf7f7f7, $darkrate = 26, $indentfactor = 7 )
{
	global $tree, $mybb, $theme, $mybb, $pid, $tid, $templates, $parser, $lang,
	$pid, $useravatar, $profilelink, $onlinestatus, $usertitle, $userstars, $groupimage, $user_details,
	$posturl, $icon, $postdate, $editedmsg, $subject_extra, $message, $attachments, $signature,
	$iplogged, $button_email, $button_pm, $button_www, $button_find, $button_rep, $button_edit,
	$button_quickdelete, $button_quickrestore, $button_quote, $button_multiquote, $button_report,
	$button_warn, $button_purgespammer, $button_reply_pm, $button_replyall_pm, $button_forward_pm,
	$button_delete_pm, $treepnumb, $post_bkg, $lastreadtime;

	$darkrate *= .8;
	$indentfactor *= .9;
	$indentsize = abs( $indentfactor * $indent);
	++$indent;
        $icr = ($indentcolor & 0xff0000) >> 16;
        $icg = ($indentcolor & 0x00ff00) >> 8;
        $icb = ($indentcolor & 0x0000ff);
        $icr -= abs( $darkrate);
        $icg -= abs( $darkrate);
        $icb -= abs( $darkrate);
        $icnew = ($icr << 16) + ($icg << 8) + $icb;
        $tmpbkg = sprintf( '%6X', $icnew);
	...
	if(is_array($tree[$replyto]))
	{
		foreach($tree[$replyto] as $key => $post)
		{
			...
			$indentsize3 = 100 - abs( $indentfactor * ($indent-1) * .2);
                        $indentsize2 = $indentsize3 - $indentsize;
			if ($indentsize3 < 0) {
                          $indentsize2 -= $indentsize3;
                          // XXXX! ccheck
                          $indentsize3 = 0;
			}
			if ($lastreadtime) {
                          $post_bordercolor = ($lastreadtime < $post['postdate']) ? 'FF0000' : $tmpbkg;
                        } else {
                          $post_bordercolor = $tmpbkg;
                        }
                          
	                $pnumber = $treepnumb[$post['replyto']][$post['pid']];
                        $post_bkg = $tmpbkg;
			$posts .= build_postbit($post, 4, $indentsize, $indentsize2, $indentsize3, $pnumber, $post_bkg, $post_bordercolor, $lastreadtime);
 			if($tree[$post['pid']])
 			{
 				$posts .= buildtree($lastreadtime, $post['pid'], $indent, $icnew, $darkrate, $indentfactor );
 			}
 			$post = '';
		}
		--$indent;
